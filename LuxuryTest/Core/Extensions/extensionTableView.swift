...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return showFavorite ? model.getFavoriteCompanies().count : model.arrayCompany.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if showFavorite {
            let favorites = model.getFavoriteCompanies()
            guard indexPath.row < favorites.count else { return UITableViewCell() }
                company = favorites[indexPath.row]
        } else {
